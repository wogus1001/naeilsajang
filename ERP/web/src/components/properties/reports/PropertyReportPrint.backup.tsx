...
const PropertyReportPrint: React.FC<PropertyReportPrintProps> = ({ data, format }) => {
    ...
    const printStyles = `
        .report-container {
            ...
            width: 210mm; /* Force exact A4 width on screen */
            min-height: 297mm; /* Force A4 height ratio visual */
            padding: 10mm 10mm !important; /* Visual margin matches print exactly */
        ...
        @media print {
            ...
            .report-container {
                position: absolute; /* Break out of parent constraints */
                top: 0;
                left: 0;
                width: 100% !important; /* Use 100% to fill whatever the page width is */
                min-height: 297mm;
                padding: 10mm 10mm !important; /* Top/Bottom 10mm, Left/Right 10mm as requested */
                margin: 0 !important;
                border: none;
            ...
            /* Format 5 Specific Optimizations for A4 Fit (Strict Single Page) */
            .report-container.format-5-root {
                padding: 5mm 10mm !important; /* Format 5 gets tighter vertical (5mm) but consistent horizontal (10mm) */
                height: 297mm !important; /* Strict height for Format 5 */
                overflow: hidden !important; /* Strict clipping for Format 5 */
            }

            .map-container-format5 {
                height: 480px !important; /* Significantly increased to fill A4 bottom */
            }
            .format5-compact-margin {
                margin-bottom: 25px !important; /* Relaxed spacing */
                gap: 25px !important; /* Relaxed grid gap */
            }
            .format5-header {
                padding-bottom: 20px !important;
                margin-bottom: 30px !important;
            }
            /* Reduce font size slightly for Format 5 tables if needed to save width/height */
            /* .report-table th, .report-table td { padding: 4px 6px !important; } */

            /* Fix map canvas cutoff by forcing it to fill container */
    ...
    const renderFormat3 = () => (
        <>
            ...
            {/* 4. Memos + Map Combined (Modified for Format 3) */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '0px' }}>
                {/* Left Column: Memos */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    <div>
                        <h2 style={{ fontSize: '18px', borderLeft: '4px solid #82c91e', paddingLeft: '10px', marginBottom: '10px', color: '#333' }}>영업현황 메모</h2>
                        <div style={{ ...getBoxStyle('150px', 7), backgroundColor: '#f4fce3', borderColor: '#d8f5a2' }}>{data.operationMemo || '-'}</div>
                    </div>
                    <div>
                        <h2 style={{ fontSize: '18px', borderLeft: '4px solid #228be6', paddingLeft: '10px', marginBottom: '10px', color: '#333' }}>임대차권리 메모</h2>
                        <div style={{ ...getBoxStyle('150px', 7), backgroundColor: '#e7f5ff', borderColor: '#d0ebff' }}>{data.leaseMemo || '-'}</div>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    {/* Width 100% fills the parent column (which is relative to A4) for correct WYSIWYG */}
                    <div style={{ width: '100%', height: '480px', border: '1px solid #dee2e6', borderRadius: '4px', overflow: 'hidden', position: 'relative' }}>
                        {(data.coordinates && data.coordinates.lat && data.coordinates.lng) ? (
                            ...
                            >
                                ...
                                <div className="print-hide" style={{
                                    ...
                                    padding: '5px 4px', // Taller padding for vertical
                                    borderRadius: '20px',
    ...
    return (
        <div className={`report-container${format === '5' ? ' format-5-root' : ''}`}>
