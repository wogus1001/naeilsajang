...
def run_once(page, url, pl, timeout_ms, resp_substr):
    ...
    # ====================================================================
    # ▼▼▼ [수정됨] 서버 응답 시간(server_ms) 측정 로직 제거 ▼▼▼
    prev_url = page.url
    server_ms = None  # server_ms는 항상 None으로 유지 (결과 CSV 컬럼 유지를 위해)

    # 타이머를 클릭 직전에 시작
    ui_start = time.perf_counter() 
    
    # 서버 응답 분리 로직(if resp_substr)을 제거하고, 클릭만 수행
    ...
    if ui_ms is None:
        ...
        # 서버 응답(server_ms)이 오지 않았다면 UI 시간은 서버 시간으로 대체 (서버 타임OUT 케이스)
...
def run(url, xlsx, runs, headless, timeout_ms, resp_substr, block_static_heavy):
        ...
        for sc in scenarios:
            ...
            for i in range(runs):
                try:
                    ...
                    print(f"[{name}] {i+1}/{runs}: E2E_SRT={ui_ms} ms")
                except Exception as e:
                ...
                rows.append({
                    ...
                    "server_srt_ms": servers[-1], # 값은 항상 None이지만 CSV 컬럼 유지를 위해 포함
    ...
    # ====================================================================
    # ▼▼▼ [수정됨] CSV 저장을 Excel(.xlsx) 저장으로 변경 (한글 깨짐 해결) ▼▼▼
    
    # 'rows' (list of dicts)를 Pandas DataFrame으로 변환
    df = pd.DataFrame(rows)

    # .xlsx 파일로 저장 (index=False로 불필요한 인덱스 열 제거)
    out_xlsx = f"match_e2e_results_{ts}.xlsx"
    try:
        # Excel 저장을 위해 'openpyxl' 엔진 사용
        df.to_excel(out_xlsx, index=False, engine='openpyxl')
        print(f"Saved: {out_xlsx}")
    except ImportError:
        # 'openpyxl'이 설치되지 않은 경우
        print(f"'{out_xlsx}' 저장 실패. 'openpyxl' 라이브러리가 필요합니다.")
...
if __name__ == "__main__":
    ...
    # ▼▼▼ [수정됨] 변수명 오타 수정 (RESPONSE_STR -> RESPONSE_SUBSTR) ▼▼▼
