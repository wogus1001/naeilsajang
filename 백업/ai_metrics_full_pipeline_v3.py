import sys, time, json, statistics, pandas as pd, requests, os
...
def run_measure(gt_csv: str, out_log: str):
    ...
    for _, r in df.iterrows():
        ...
        # ✅ 더 이상 *Label, isFranchise 타입을 건드리지 않음
        # (브라우저와 동일 포맷을 그대로 POST)
        ...
        if st != 200:
            rows.append({"label":label,"status":st,"hit":0,"latency_ms":round(lat,1),
                         "expected_id":expected_id,"top5_ids":[],"top5_names":[],
...
def main():
    if len(sys.argv) < 2:
        print("USAGE: python ai_metrics_full_pipeline_v3.py <ground_truth_csv>")
        sys.exit(1)
    gt = sys.argv[1]
    out_log = "ai_metric_log_all.csv"
    print(f">> USING GT FILE: {gt}")
    log_all, acc, avg, p95v, hits, total, stc = run_measure(gt, out_log)
    print("\n===== [all] 결과 요약 =====")
